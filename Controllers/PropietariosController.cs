...
public IActionResult Delete(int id, Propietario propietario)
{
    ...
    if (rowsAffected > 0)
    {
        TempData["SuccessMessage"] = $"El propietario con ID {id} ha sido eliminado exitosamente.";
    }
    else
    {
        TempData["ErrorMessage"] = $"No se pudo encontrar el propietario con ID {id} para eliminar.";
    }
