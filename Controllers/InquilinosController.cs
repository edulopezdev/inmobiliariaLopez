...
public IActionResult Delete(int id, Inquilino Inquilino)
{
    ...
    if (rowsAffected > 0)
    {
        TempData["SuccessMessage"] = $"El Inquilino con ID {id} ha sido eliminado exitosamente.";
    }
    else
    {
        TempData["ErrorMessage"] = $"No se pudo encontrar el Inquilino con ID {id} para eliminar.";
    }
